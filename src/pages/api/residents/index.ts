...
const getResidents = withAdmin(async (req, res: NextApiResponse<ApiResponse>) => {
  await limiter(req, res, async () => {
    try {
      ...
      console.error('[residents/list]', err);
      return res.status(500).json({ success: false, error: 'Failed to fetch residents' });
    }
  });
});
...
const registerResident = withToken(async (req, res: NextApiResponse<ApiResponse>) => {
  await limiter(req, res, async () => {
    try {
      ...
      console.error('[residents/register]', err);
      return res.status(500).json({ success: false, error: 'Failed to register resident' });
    }
  });
});
